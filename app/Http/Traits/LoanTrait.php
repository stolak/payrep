...
trait LoanTrait
{
    public static function implimentDefaulter($id)
    {
        $loan = Loan::find($id);
        if (!$loan) return ;
    ...
    public static function nextCycle($date)
    {
        $dt = date_create($date);
        date_modify($dt, '+1 month');
        return date_format($dt, 'Y-m-d');
    }
    public static function prevCycle($date)
    {
        $dt = date_create($date);
        date_modify($dt, '-1 month');
        return date_format($dt, 'Y-m-d');
    }
    public static function computeBackLog($id)
    {
        $loan = Loan::find($id);
    ...
    public static function computeDueLoan($date)
    {
    ...
    public static function computeBackLog2($id, $date)
    {
        ...
        public static function computeBalance($id)
        {
        ...
        public static function nextInterest($id)
        {
            if (Loan::where('id', $id)->first()->loan_type_id==2) {
                $loan = Loan::find($id);
                $debit = $loan->total_interest/$loan->period;
        ...
        public static function nextRepayment($id)
        {
            $debit = self::nextInterest($id);
             $loan = Loan::find($id);
            $monthly = $loan->amount_approved/$loan->period;
            
            return $monthly+$debit;
        }
}