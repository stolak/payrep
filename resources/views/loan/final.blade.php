            ...
            <div class="page-wrapper">
				<div class="content container-fluid">
					...
					<div class="page-header">
						<div class="row">
							<div class="col">
								<h3 class="page-title">Final Approval</h3>
								<ul class="breadcrumb">
											...
											<tbody>
											   ...
											   @foreach($Loans as $list)
												<tr>
													...
													@else
													{{number_format($list['amount_approved']*$list['percentage']*0.01,2, '.', ',')}}
													@endif
													</td>
													<td >
													@if($list['loan_type_id']==2)
													{{number_format(($list['amount_approved']/$list['period'])+($list['total_interest']/$list['period']),2, '.', ',')}}
													@else
													{{number_format(($list['amount_approved']/$list['period'])+($list['amount_approved']*$list['percentage']*0.01),2, '.', ',')}}
													@endif
													</td>
													<td >
													...
													<td>
														...
														href="javascript: editFunc('{{$list->id}}', '{{$list->customer_id}}', '{{$list->marketer_id}}', '{{$list->amount_approved}}', '{{$list->period}}', '{{$list->percentage}}','{{$list->loan_type_id}}')">
								...
								<div class="row ">
									<div class="col-12 col-sm-12">
										<div class="form-group">
											...
											<input type="date" class="form-control" name="approvalDate" value="{{date('Y-m-d')}}">
										</div>
									</div>
							    </div>
								<div class="row ">
