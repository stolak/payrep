            ...
            <div class="page-wrapper">
				<div class="content container-fluid">
					...
					<div class="page-header">
						<div class="row">
							<div class="col">
								<h3 class="page-title">Account Officer Review</h3>
								<ul class="breadcrumb">
											...
											<tbody>
											   ...
											   @foreach($Loans as $list)
												<tr>
													...
													<td >
													{{number_format($list->amount_marketer,2, '.', ',')}}
													</td>
													<td >
													{{$list['particular']}}
													</td>
													...
													@else
													{{number_format($list['amount_marketer']*$list['percentage']*0.01,2, '.', ',')}}
													@endif
													</td>
													<td >
													@if($list['loan_type_id']==2)
													{{number_format(($list['amount_marketer']/$list['period'])+($list['total_interest']/$list['period']),2, '.', ',')}}
													@else
													{{number_format(($list['amount_marketer']/$list['period'])+($list['amount_marketer']*$list['percentage']*0.01),2, '.', ',')}}
													@endif
													</td>
													<td >
													...
													<td>
														...
														href="javascript: editFunc('{{$list->id}}', '{{$list->customer_id}}', '{{$list->marketer_id}}', '{{$list->amount_marketer}}', '{{$list->period}}', '{{$list->percentage}}','{{$list->loan_type_id}}')">
